    ...
    def play_match(self):
        ...
        while not self.is_match_over():
            ...
            while not self.current_game.is_game_over():
                ...
                column_match = self.current_game.rank_in_hand(current_player, last_discarded)
                ...
                if column_match is not None:
                    ...
                    swap_card = (row, col)
                    logger.debug(f"Discarded card in hand: {current_player.name} is trying to match a column by swapping {card_to_discard} with the {last_discarded} on the discard pile")
                else:
                    ...
                    if highest_card is not None and highest_card[0].get_value() > last_discarded.get_value():
                        ...
                        if last_discarded is not None and card_to_discard.get_value() > last_discarded.get_value():
                            logger.debug(f"{current_player.name} is swapping a high value card ({card_to_discard}) with the {last_discarded} on the discard pile")
                            take_from_deck = False
                        else:
                            # Option 3. Swap a high value card with a card from the deck
                            logger.debug(f"{current_player.name} is swapping a high value card ({card_to_discard}) with a card from the deck")
                    else:
        ...
        if winner is None:
            logger.info("Match over! - D R A W !")
        else:
            logger.info(f"Match over! - The winner is {winner.name} with a total score of {self.total_scores[winner]}!")
